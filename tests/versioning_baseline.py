...
Usage:
    python3 tests/versioning_baseline.py             # recover and write released-surface.json
    python3 tests/versioning_baseline.py --print     # print it instead, changing nothing
    python3 tests/versioning_baseline.py --rule-url  # print the pinned coordinate of the rule
"""
...
def extractor():
    ...
    if _EXTRACTOR is None:
        import wisent.surface as surface
...
def build(repo: pathlib.Path) -> dict:
    ...
    document = {
        "version": version,
        "source": f"{MARKER}:{filename} unpacked and read by python3 -m wisent.surface",
        "surface": names,
    }
    if skipped:
